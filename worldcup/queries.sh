...
echo "$($PSQL "SELECT ROUND(AVG(winner_goals) + AVG(opponent_goals),16) AS total FROM games")"
...
echo "$($PSQL "SELECT DISTINCT(name)  FROM games AS g_win  FULL JOIN teams ON g_win.winner_id = teams.team_id FULL JOIN games AS g_opp ON teams.team_id = g_opp.opponent_id WHERE (g_win.round = 'Eighth-Final' OR g_opp.round = 'Eighth-Final') AND ( g_win.year = 2014 OR g_opp.year = 2014)")"
